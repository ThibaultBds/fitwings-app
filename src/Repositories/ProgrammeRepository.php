...
class ProgrammeRepository extends BaseRepository
{
    ...
    private function tableHasDetailColumns(): bool
    {
        if ($this->hasDetailColumns !== null) {
            return $this->hasDetailColumns;
        }

        $stmt = $this->db->query("SHOW COLUMNS FROM programme LIKE 'duree_semaines'");
        $this->hasDetailColumns = (bool) $stmt->fetch();
    ...
    public function getFiltered(?string $objectif = null, ?string $niveau = null): array
    {
        ...
        if ($objectif !== '') {
            $conditions[] = 'LOWER(objectif) = :objectif';
            $params['objectif'] = $objectif;
        }

        if ($niveau !== '') {
            $conditions[] = 'LOWER(niveau) = :niveau';
