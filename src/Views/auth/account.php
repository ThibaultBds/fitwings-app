...
<main class="account-layout">
  ...
  <aside class="account-sidebar">
    ...
    <div class="account-card-label">Carte abonne</div>
    <?php if ($plan): ?>
    ...
    <?php else: ?>
      <div class="membre-card membre-card--empty">
        <div class="membre-card-no-plan">
          <p>Aucun abonnement actif</p>
          <p class="membre-card-no-plan-link"><a href="/abonnements">Voir nos offres</a></p>
        </div>
      </div>
    <?php endif; ?>
    ...
    <div class="account-actions">
      ...
      <a href="/logout" class="btn-danger">Deconnexion</a>
      <?php if (in_array($user->role, ['admin', 'moderateur'])): ?>
        <?php if ($user->role === 'admin'): ?>
          <a href="/admin" class="btn-primary">Panneau admin</a>
        <?php else: ?>
          <a href="/moderator" class="btn-primary">Espace moderation</a>
        <?php endif; ?>
      <?php endif; ?>
    </div>
  </aside>
  ...
  <section class="account-main">
    <div class="card account-section">
      <h2>Ajouter une entree de suivi</h2>
      <form method="post" action="/account/progression" class="progression-form">
        ...
        <div class="progression-fields">
          ...
          <label>Nombre de seances<input type="number" name="nombre_seances" placeholder="3" required></label>
        </div>
